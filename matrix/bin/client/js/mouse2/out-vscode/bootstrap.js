...
(function (globalThis, factory) {
	...
	//#region Add support for using node_modules.asap

	/**
	 * @param {string=} appRoot
	 */
	function enableASAPSupport(appRoot) {
		if (!path || !Module || typeof process === 'undefined') {
			console.warn('enableASAPSupport() is only available in node.js environments');
			return;
		}
		...
		const NODE_MODULES_ASAP_PATH = `${NODE_MODULES_PATH}.asap`;
		const NODE_MODULES_ASAP_ALTERNATIVE_PATH = NODE_MODULES_ALTERNATIVE_PATH ? `${NODE_MODULES_ALTERNATIVE_PATH}.asap` : undefined;
		...
		Module._resolveLookupPaths = function (request, parent) {
			...
			if (Array.isArray(paths)) {
				let asapPathAdded = false;
				for (let i = 0, len = paths.length; i < len; i++) {
					if (paths[i] === NODE_MODULES_PATH) {
						asapPathAdded = true;
						paths.splice(i, 0, NODE_MODULES_ASAP_PATH);
						break;
					} else if (paths[i] === NODE_MODULES_ALTERNATIVE_PATH) {
						asapPathAdded = true;
						paths.splice(i, 0, NODE_MODULES_ASAP_ALTERNATIVE_PATH);
						break;
					}
				}
				if (!asapPathAdded && appRoot) {
					// Assuming that adding just `NODE_MODULES_ASAP_PATH` is sufficient
					// because nodejs should find it even if it has a different drive letter case
					paths.push(NODE_MODULES_ASAP_PATH);
				}
			}
	...
	function safeProcess() {
		...
		if (sandboxGlobals) {
			return sandboxGlobals.process; // Native environment (sandbox)
		}

		if (typeof process !== 'undefined') {
			return process; // Native environment (non-sandbox)
		}
	...
	// Prevents insights from monkey patching modules.
	...
	function avoidMonkeyPatchFromAppInsights() {
		...
		// @ts-ignore
		process.env['APPLICATION_INSIGHTS_NO_DIAGNOSTIC_CHANNEL'] = true; // Skip monkey patching of 3rd party modules by insights
	...
	return {
		enableASAPSupport: enableASAPSupport,
