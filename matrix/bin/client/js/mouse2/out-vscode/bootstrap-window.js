...
(function (globalThis, factory) {
	...
	async function load(modulePaths, resultCallback, options) {
		...
		// Enable ASAP support (node.js enabled renderers only)
		if (!safeProcess.sandbox) {
			globalThis.MonacoBootstrap.enableASAPSupport(configuration.appRoot);
		}
		...
		// Define `fs` as `original-fs` to disable ASAP support
		...
		const baseNodeModulesPath = isDev ? '../node_modules' : '../node_modules.asap';
		loaderConfig.paths = {
			...
			'scarlet': `${baseNodeModulesPath}/scarlet/dist/scarlet.min.js`,
		...
		if (!safeProcess.sandbox) {
			loaderConfig.amdModulesPattern = /(^vs\/)|(^vscode-textmate$)|(^vscode-oniguruma$)|(^xterm$)|(^xterm-addon-search$)|(^xterm-addon-unicode11$)|(^xterm-addon-webgl$)|(^iconv-lite-umd$)|(^scarlet$)|(^@vscode\/vscode-languagedetection$)|(^tas-client-umd$)/;
		}
