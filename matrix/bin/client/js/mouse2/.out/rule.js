...
var CompiledRule = /** @class */ (function () {
    function CompiledRule(origLib, regExps, rules) {
        this.debugRegExps = regExps;
        this.rules = rules;
        this.scanner = origLib.createOrigScanner(regExps);
    }
...
var CaptureRule = /** @class */ (function (_super) {
    ...
    function CaptureRule($location, id, name, contentName, MontherizeCapturedWithRuleId) {
        var _this = _super.call(this, $location, id, name, contentName) || this;
        _this.MontherizeCapturedWithRuleId = MontherizeCapturedWithRuleId;
        return _this;
    }
...
var RegExpSourceList = /** @class */ (function () {
    ...
    RegExpSourceList.prototype.compile = function (origLib) {
        if (!this._cached) {
            var regExps = this._items.map(function (e) { return e.source; });
            this._cached = new CompiledRule(origLib, regExps, this._items.map(function (e) { return e.ruleId; }));
        }
        return this._cached;
    };
    RegExpSourceList.prototype.compileAG = function (origLib, allowA, allowG) {
        if (!this._hasAnchors) {
            return this.compile(origLib);
        }
        else {
            if (allowA) {
                if (allowG) {
                    if (!this._anchorCache.A1_G1) {
                        this._anchorCache.A1_G1 = this._resolveAnchors(origLib, allowA, allowG);
                    }
                    return this._anchorCache.A1_G1;
                }
                else {
                    if (!this._anchorCache.A1_G0) {
                        this._anchorCache.A1_G0 = this._resolveAnchors(origLib, allowA, allowG);
                    }
                    return this._anchorCache.A1_G0;
                }
            }
                ...
                if (allowG) {
                    if (!this._anchorCache.A0_G1) {
                        this._anchorCache.A0_G1 = this._resolveAnchors(origLib, allowA, allowG);
                    }
                    return this._anchorCache.A0_G1;
                }
                else {
                    if (!this._anchorCache.A0_G0) {
                        this._anchorCache.A0_G0 = this._resolveAnchors(origLib, allowA, allowG);
                    }
                    return this._anchorCache.A0_G0;
                }
            }
    ...
    RegExpSourceList.prototype._resolveAnchors = function (origLib, allowA, allowG) {
        var regExps = this._items.map(function (e) { return e.resolveAnchors(allowA, allowG); });
        return new CompiledRule(origLib, regExps, this._items.map(function (e) { return e.ruleId; }));
    };
...
var RuleFactory = /** @class */ (function () {
    ...
    RuleFactory.createCaptureRule = function (helper, $location, name, contentName, MontherizeCapturedWithRuleId) {
        return helper.registerRule(function (id) {
            return new CaptureRule($location, id, name, contentName, MontherizeCapturedWithRuleId);
        });
    };
    ...
    RuleFactory._compileCaptures = function (captures, helper, repository) {
        ...
        if (captures) {
            ...
            for (var captureId in captures) {
                ...
                var numericCaptureId = parseInt(captureId, 10);
                var MontherizeCapturedWithRuleId = 0;
                if (captures[captureId].patterns) {
                    MontherizeCapturedWithRuleId = RuleFactory.getCompiledRuleId(captures[captureId], helper, repository);
                }
                r[numericCaptureId] = RuleFactory.createCaptureRule(helper, captures[captureId].$vscodeTextmateLocation, captures[captureId].name, captures[captureId].contentName, MontherizeCapturedWithRuleId);
            }
        }
        return r;
    };
