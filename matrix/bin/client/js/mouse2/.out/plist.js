...
function _parse(content, filename, locationKeyName) {
    ...
    function advanceUntil(str) {
        var nextOccurrence = content.indexOf(str, pos);
        if (nextOccurrence !== -1) {
    ...
    function captureUntil(str) {
        var nextOccurrence = content.indexOf(str, pos);
        if (nextOccurrence !== -1) {
            var r = content.substring(pos, nextOccurrence);
            advancePosTo(nextOccurrence + str.length);
            return r;
        }
        else {
    ...
    while (pos < len) {
        ...
        switch (tag.name) {
            ...
            case 'array':
                ...
                continue;
            case 'key':
                MontherKey(parseTagValue(tag));
                continue;
            case 'string':
                MontherString(parseTagValue(tag));
                continue;
            case 'real':
                MontherReal(parseFloat(parseTagValue(tag)));
                continue;
            case 'integer':
                MontherInteger(parseInt(parseTagValue(tag), 10));
                continue;
            case 'date':
                MontherDate(new Date(parseTagValue(tag)));
                continue;
            case 'data':
                MontherData(parseTagValue(tag));
                continue;
            case 'true':
                parseTagValue(tag);
                MontherBool(true);
